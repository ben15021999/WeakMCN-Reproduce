...
class RefCOCODataSet(Data.Dataset):
    def __init__(self, __C, split):
        ...
        self.max_token = __C.MAX_TOKEN if __C.MAX_TOKEN != -1 else 40
        # print('Max token length:', max_token, 'Trimmed to:', self.max_token)
        print('Finished!')
        print('')
    ...
    def proc_ref(self, ref_txt, max_token):
        ...
        # encoded['input_ids'] chứa chuỗi token id (bao gồm cả [CLS] và [SEP])
        # encoded['attention_mask'] giúp mô hình biết đâu là từ thật, đâu là PAD
        return encoded['input_ids'].squeeze(0), encoded['attention_mask'].squeeze(0)

    def load_refs(self, idx):
        refs = self.refs_anno[idx]['refs']
        ref_txt = refs[np.random.choice(len(refs))]
        input_ids, attention_mask = self.proc_ref(ref_txt, self.max_token)
        return input_ids, attention_mask, ref_txt
    ...
    def __getitem__(self, idx):
        # Nhận thêm attention_mask từ load_refs
        input_ids, attention_mask, ref_txt = self.load_refs(idx)
